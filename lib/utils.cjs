...
const getPage = async (word) => {
    const url$1 = url.resolve(DICTIONNARY_URL, word);
    let response;
    try {
        response = await axios.get(url$1);
    } catch (error) {
        const message = `Failed to fetch page ${url$1}, server responded with ${error.response.status}`;
...
const getSuggestions = (content, exist) => {
    ...
    content(selector).each((_, elem) => {
        const suggestion = {
            ...
            url: url.resolve(LAROUSSE_URL, elem.attribs.href)
        };
    ...
    onymContent.children.forEach((childOnyms) => {
        if (childOnyms.type == "text" && childOnyms.data != " - ")
            ...
            const onym = {
                ...
                url: url.resolve(LAROUSSE_URL,childOnyms.children[0].attribs.href)
            };
